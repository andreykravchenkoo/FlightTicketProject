...
class AirportControllerTest {
    ...
    void testShouldFindAirportsByCity() throws Exception {
        ...
        List<AirportInfoDto> expectedAirports =
                List.of(new AirportInfoDto("LOND", "London City", expectedCity, "UK"));

        when(goflightlabsClientService.findAirportByCity("London")).thenReturn(expectedAirports);

        MvcResult result =
                mockMvc.perform(
                                MockMvcRequestBuilders.get("/api/airports/search")
                                        .param("city", expectedCity))
                        .andExpect(status().isOk())
                        .andReturn();

        String responseBody = result.getResponse().getContentAsString();
        List<AirportInfoDto> airports =
                objectMapper.readValue(responseBody, new TypeReference<List<AirportInfoDto>>() {});
