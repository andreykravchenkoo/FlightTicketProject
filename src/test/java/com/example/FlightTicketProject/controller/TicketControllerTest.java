...
class TicketControllerTest {
    ...
    void testShouldFindAllTickets() throws Exception {
        List<TicketDto> expectedTicketsDto = List.of(new TicketDto(1L, "Test Test", "F10"));

        List<Ticket> expectedTickets = List.of(expectedTicketsDto.get(0).toEntity());
        ...
        List<TicketDto> tickets =
                objectMapper.readValue(responseBody, new TypeReference<List<TicketDto>>() {});
    ...
    void testShouldFindAllTicketsByUser() throws Exception {
        ...
        MvcResult result =
                mockMvc.perform(MockMvcRequestBuilders.get("/api/tickets/user"))
                        .andExpect(status().isOk())
                        .andReturn();
    ...
    void testShouldFindTicketById() throws Exception {
        ...
        MvcResult result =
                mockMvc.perform(MockMvcRequestBuilders.get("/api/tickets/{ticketId}", expectedId))
                        .andExpect(status().isOk())
                        .andReturn();

        String responseBody = result.getResponse().getContentAsString();
        TicketDto ticketDto =
                objectMapper.readValue(responseBody, new TypeReference<TicketDto>() {});
