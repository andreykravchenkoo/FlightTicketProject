...
class FlightControllerTest {
    ...
    void testShouldFindAllFlight() throws Exception {
        List<FlightDto> expectedFlightsDto = List.of(
                new FlightDto("1", "London", "Berlin", new Date(), new Date(), 100, FareClassStatus.ECONOMY, "Iberia Express")
        );

        List<Flight> expectedFlight = List.of(
                expectedFlightsDto.get(0).toEntity()
        );
        ...
        List<FlightDto> flights = objectMapper.readValue(responseBody, new TypeReference<List<FlightDto>>(){});
    ...
    void testShouldFindFlightsByFilter() throws Exception {
        ...
        Set<FlightDto> expectedFlightsDto = Set.of(
                new FlightDto("1", "London", "Berlin", new Date(), new Date(), 100, FareClassStatus.ECONOMY, "Iberia Express")
        );

        when(goflightlabsClientService.findFlightsByFilter(expectedAdults, expectedOrigin, expectedDestination, expectedDepartureDate, expectedFareClass))
                .thenReturn(expectedFlightsDto);

        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.get("/api/flights/search")
    ...
    void testShouldFindFlightById() throws Exception {
        ...
        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.get("/api/flights/{flightId}", expectedId))
                .andExpect(status().isOk())
                .andReturn();

        String responseBody = result.getResponse().getContentAsString();
        FlightDto flightDto = objectMapper.readValue(responseBody, new TypeReference<FlightDto>(){});
    ...
    void testShouldSaveFlight() throws Exception {
        ...
        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.post("/api/flights")
