...
class PaymentControllerTest {
    ...
    void testShouldFindAllPayments() throws Exception {
        List<PaymentDto> expectedPaymentsDto = List.of(
                new PaymentDto(1L, "Test Test", 100, new Date(), PaymentStatus.NEW)
        );

        List<Payment> expectedPayments = List.of(
                expectedPaymentsDto.get(0).toEntity()
        );

        when(paymentService.findAll()).thenReturn(expectedPayments);

        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.get("/api/payments"))
                .andExpect(status().isOk())
                .andReturn();

        String responseBody = result.getResponse().getContentAsString();
        List<PaymentDto> payments = objectMapper.readValue(responseBody, new TypeReference<List<PaymentDto>>(){});
    ...
    void testShouldFindAllPaymentsByUser() throws Exception {
        ...
        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.get("/api/payments/user"))
                .andExpect(status().isOk())
                .andReturn();
    ...
    void testShouldFindPaymentById() throws Exception {
        ...
        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.get("/api/payments/{paymentId}", expectedId))
                .andExpect(status().isOk())
                .andReturn();

        String responseBody = result.getResponse().getContentAsString();
        PaymentDto paymentDto = objectMapper.readValue(responseBody, new TypeReference<PaymentDto>(){});
    ...
    void testShouldExecutePayment() throws Exception {
        ...
        when(paymentProcessorFacade.executePayment(expectedPaymentId, expectedSum)).thenReturn(expectedPayment);

        MvcResult result = mockMvc.perform(MockMvcRequestBuilders.post("/api/payments/execute")
                        .param("paymentId", "1")
                        .param("sum", "100"))
                .andExpect(status().isOk())
                .andReturn();
